...
public class ItemLivingWeedwoodShield extends ItemWeedwoodShield {
	...
	protected boolean trySpit(ItemStack stack, World world, EntityLivingBase owner, EnumHand hand) {
		...
		if(target != null) {
			...
			Vec3d offset = new Vec3d(bodyForward.x, owner.getEyeHeight(), bodyForward.z).add(right.scale(hand == EnumHand.MAIN_HAND ? 0.35D : -0.35D).addVector((double) 0, lookVec.y * 0.5D - 0.4D, (double) 0).add(bodyForward.scale(-0.1D)));
